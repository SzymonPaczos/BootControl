...
| P1.1 — Dwie niezależne blacklisty | ⏭️ Backlog | Cross-crate refaktor: `BLACKLISTED_PATTERNS` przenieść do `bootcontrol_core::security` (nowy moduł), `daemon::sanitize::check_payload` + `core::backends::uki::validate_kernel_param` delegate do tej samej stałej. Wymaga przemyślenia interfejsu (substring check vs full-param check). Backlog P1. |
