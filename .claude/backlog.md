...
### `crates/daemon` nie kompiluje się na Linuksie (41 dni na `main`)
`polkit.rs:85-86` używa `actions::GENERATE_KEYS` i `actions::REPLACE_PK`, usuniętych z modułu `actions` commitem `4fcf14c` (2026-07-12) — zostały tylko osierocone doc-komentarze (`polkit.rs:28-29`). Dowód: `cargo check -p bootcontrold --lib --target x86_64-unknown-linux-gnu` → 2× `error[E0425]`. To samo dotyczy testu `polkit.rs:171,174` oraz `policy_check.rs:177-195` (indeksy `REQUIRED_ACTIONS[4]`/`[5]` na 4-elementowej tablicy, `assert_eq!(missing.len(), 3)` przy realnym 1). **Naprawa = usunąć obie pozycje**, nie przywracać stałych: to akcje skasowane decyzją „Zakres i kolejka 1.0".
**Źródło:** audyt 2026-08-22, Security Reviewer F2 (HIGH) + weryfikacja cross-compile. **Status:** otwarte.
...
**Źródło:** audyt 2026-08-22 (warstwa głęboka). **Status:** otwarte.
...
**Źródło:** audyt 2026-08-22, Security Reviewer F6. Rozszerza zamknięty P2 „polkit 5→6 drift" z 2026-07-12. **Status:** otwarte.
