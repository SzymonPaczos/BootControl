...
### GUI: Confirmation Sheet pokazuje stub-owy diff i preflight „all pass" także poza Demo Mode
`crates/gui/src/main.rs:158-165` — `open_confirmation("rewrite-grub")` wypełnia sheet przez `build_stub_diff()` i `build_stub_preflight_passing()` **bezwarunkowo** (komentarze: „PR 5 daemon runs real checks" — nigdy nie podpięte). Na realnym systemie użytkownik widzi zmyślony diff `/etc/default/grub` i cztery „zaliczone" preflight checki („287 MB free", „ESP mounted"), których nikt nie wykonał — kategoria „kod kłamiący użytkownika" (P0 wg definicji audytu, gdy write-path aktywny). Fix wchodzi naturalnie w Tor A/A1 (diff/preflight z daemona); do tego czasu rozważyć oznaczenie sheeta w trybie non-demo jako „preview unavailable". Restore-flow ma pokrewny stub: `main.rs:267` (pusty diff + jeden passing check).
**Źródło:** przegląd placeholderów przy handoffie Tor B, sesja 2026-07-27 ([findings](../task-briefs/gui-ux-redesign.md) → materiał DesignSync `findings/alignment-findings`). **Status:** otwarte — zapisane, nie rozpoczęte.
