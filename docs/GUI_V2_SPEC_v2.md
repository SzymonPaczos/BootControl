...
**Verdict** *(moot since 2026-07-12 — Paranoia/Strict Mode removed by the "1.0 GRUB-first" scope decision; kept for the record)*. Strict Mode (`experimental_paranoia` feature) is a single inline disclosure on the Secure Boot page. Inside it, every destructive op (Replace PK, Generate keys, Erase enrolled keys) requires type-to-confirm with the full action name (e.g. user types `Replace platform key`). **Additionally**, a runtime policy file `/etc/bootcontrol/policy.toml` with `strict_mode_allowed = false` (default) blocks the disclosure from rendering at all even if the daemon was compiled with `experimental_paranoia`.
