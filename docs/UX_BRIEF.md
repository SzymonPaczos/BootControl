...
3. On all-green, GUI invokes the daemon's privileged method. The daemon uses four active per-intent actions: `org.bootcontrol.write-bootloader`, `org.bootcontrol.enroll-mok`, `org.bootcontrol.rewrite-grub`, `org.bootcontrol.restore-snapshot` (canonical list: `packaging/polkit/org.bootcontrol.policy` and `polkit::KNOWN_ACTIONS`). Historical `generate-keys` and `replace-pk` were removed with Paranoia Mode and are refused by authorization. For active sessions, `rewrite-grub`, `write-bootloader` and `restore-snapshot` use `auth_admin_keep`; `enroll-mok` uses `auth_admin`. Inactive and other sessions use `auth_admin` for all four actions. Each shipped policy action has its own contextual authentication message.
