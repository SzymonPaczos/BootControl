...
| Polkit Action IDs | 4 active per-intent (`rewrite-grub`, `write-bootloader`, `enroll-mok`, `restore-snapshot`) | Single source of truth: [`packaging/polkit/org.bootcontrol.policy`](./packaging/polkit/org.bootcontrol.policy) + [`crates/daemon/src/polkit.rs`](./crates/daemon/src/polkit.rs) `KNOWN_ACTIONS`. Historical `generate-keys` / `replace-pk` were removed with Paranoia Mode and are refused by authorization, as is legacy `org.bootcontrol.manage`. A legacy manage-only policy is refused at daemon startup. |
