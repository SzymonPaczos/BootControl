...
1. **Polkit authorization** — `polkit.rs::check_authorized(...)` against the per-intent action (one of four — see [`crate::polkit::actions`](./src/polkit.rs); `generate-keys`/`replace-pk` removed with Paranoia Mode, scope decision 2026-07-12). Reject before any `read()`.
