...
async fn load_entries_from_daemon() -> anyhow::Result<()> {
    ...
    view_model.load().await.expect("Failed to load entries from daemon");
    
    // Assert entries populated
    assert_eq!(view_model.entries.get("GRUB_DEFAULT").map(|s| s.as_str()), Some("0"));
    assert_eq!(view_model.entries.get("GRUB_TIMEOUT").map(|s| s.as_str()), Some("5"));
...
async fn commit_edit_calls_set_grub_value() -> anyhow::Result<()> {
    ...
    // Attempt edit
    view_model.commit_edit("GRUB_TIMEOUT", "10").await.expect("Failed to commit edit");
...
async fn stale_etag_shows_error() -> anyhow::Result<()> {
    ...
    vm1.commit_edit("GRUB_TIMEOUT", "42").await.expect("vm1 commit failed");
    
    // vm2 attempts to commit with a stale etag
    let res = vm2.commit_edit("GRUB_DEFAULT", "1").await;
    
    assert!(res.is_err(), "Expected StateMismatch error due to stale ETag but got success");
    let err_str = res.unwrap_err().to_string();
    assert!(err_str.contains("StateMismatch") || err_str.contains("stale"), "Unexpected error type: {}", err_str);
